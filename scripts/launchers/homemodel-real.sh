#!/usr/bin/env bash
# HomeModel – real mode launcher
#
# Starts the backend in real mode using a persistent SQLite database, waits
# until the backend is reachable, then opens Firefox at http://127.0.0.1:8000.
# Closing the terminal or pressing Ctrl-C stops the backend cleanly.
#
# Usage:
#   bash <SCRIPT_DIR>/homemodel-real.sh
#
...
DB_PATH="$DATA_DIR/homemodel.db"
APP_URL="http://127.0.0.1:8000"

# ── Environment ──────────────────────────────────────────────────────────────
export HOMEMODEL_MODE=real
export SCHEMASTORE_DB_PATH="$DB_PATH"
...
# ── Start backend ─────────────────────────────────────────────────────────────
cd "$REPO_ROOT"
echo "Starting HomeModel backend in real mode (DB: $DB_PATH)…"
