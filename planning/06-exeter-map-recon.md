...
| **Parcels for Identify** | **5504** | **9499** | **Queryable, hidden — use for GetFeatureInfo** |
...
## JS API (for Playwright fallback)
...
## Mapping to homemodel Entity Types
...
- Scene origin (lat 42.98743, lon -70.98709) falls within Exeter town boundary — confirmed by BBOX overlap
- EPSG:3437 → WGS84 transform required: use `pyproj.Transformer.from_crs(3437, 4326)`
- WMS endpoint is public (no auth required); rate-limit requests
- `GetCapabilities` response should be cached to avoid repeated discovery calls
- `INFO_FORMAT=application/json` confirmed supported by MapServer 7.x
- Parcel layer `sl_id 9499` is hidden by default but queryable — ideal for GetFeatureInfo
